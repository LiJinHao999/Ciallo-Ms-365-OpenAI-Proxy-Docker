...
def _chat_stream_extra_kwargs(client: object, extra: dict) -> dict:
    """Keep sources/reasoning kwargs only when the inner client accepts them."""
    target = client
    for attr in ("_throttle_target", "_client"):
        inner = getattr(target, attr, None)
        if inner is not None and hasattr(inner, "chat_stream"):
            target = inner
    func = getattr(target, "chat_stream", None)
    try:
    ...
    if any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
        return extra
    return {key: value for key, value in extra.items() if key in sig.parameters}


async def _compat_chat_stream(
    ...
    **extra,
    ...
    """Call chat_stream, ignoring extra kwargs older/test clients reject."""
    filtered = _chat_stream_extra_kwargs(client, extra)
    stream = client.chat_stream(
        prompt, additional_context, session, images, **filtered
    )
...
async def _openai_stream(
    ...
    try:
        async for delta in _compat_chat_stream(
            client,
            prompt,
            additional_context,
            session,
        ...
        if tail:
            yield f"data: {json.dumps({'id': completion_id, 'object': 'chat.completion.chunk', 'created': created, 'model': model_alias, 'choices': [{'index': 0, 'delta': {'content': tail}, 'finish_reason': None}]})}\n\n"
...
async def _responses_stream(
    ...
    sequence += 1
    output_items = ([reasoning_item] if reasoning_item else []) + [message_item]
    completed = _responses_object(
        resp_id, model_alias, created, "completed", output_items,
        response_tools=response_tools,
        tool_choice=tool_choice,
        parallel_tool_calls=parallel_tool_calls,
        previous_response_id=previous_response_id,
...
async def _anthropic_stream(
    ...
    try:
        ...
        if isinstance(exc, SubstrateThrottled):
            ...
            return
        yield sse("content_block_delta", {"type": "content_block_delta", "index": text_index, "delta": {"type": "text_delta", "text": error_text}})
