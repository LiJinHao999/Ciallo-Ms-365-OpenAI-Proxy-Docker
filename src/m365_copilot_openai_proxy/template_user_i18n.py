...
_USER_I18N_JS = """const i18n={
  zh:{
    ...
    qs_title:'快速使用指南',qs_body:'1. 推荐：在账户卡片点「微软登录」，浏览器登录后粘贴带 code= 的回调 URL 完成绑定（无需油猴）。<br>2. 可选：安装 <a href="https://gh-proxy.com/https://raw.githubusercontent.com/MurasameCyan/Ciallo-Ms-365-OpenAI-Proxy-Docker/multi/get_token.user.js" target="_blank" rel="noopener" class="qs-link">油猴脚本</a> 推送 Cookie，用于图片/附件等媒体增强。<br>3. 复制 Base URL 与 API Key，填入 OpenAI 兼容客户端即可使用。',
    ...
    oauth_title:'微软登录（推荐）',oauth_hint:'点击后在浏览器登录微软账号。若页面跳到 wrongplace，请复制仍带 code= 的完整地址并粘贴到下方。登录成功后会自动刷新状态，无需手动点刷新。',oauth_start_btn:'微软登录',oauth_submit_btn:'提交回调 URL',oauth_starting:'打开登录页...',oauth_submitting:'提交中...',oauth_ok:'登录成功，已自动续期',oauth_failed:'登录失败',oauth_ph:'粘贴 https://login.microsoftonline.com/common/oauth2/nativeclient?code=...&state=...',
    login_mode_none:'未登录',login_mode_oauth:'微软登录(OAuth)',login_mode_rt:'RT 自动续期',login_mode_cookie:'Cookie 会话',login_mode_token:'仅 Token',
    refresh_auto_rt:'自动(RT)',refresh_auto:'自动(CDP)',refresh_manual:'手动',refresh_unavailable:'不可用',
    rt_ready:'RT 已就绪 · 自动续期',rt_missing:'无 RT · 过期需重登',
    login_ready_hint:'已绑定 Microsoft。文本对话会在请求时自动用 RT 续期，一般不用手动刷新。Cookie 绿灯只影响生图/附件增强，不是登录失败。',
    login_token_only_hint:'已有 Token，但缺少 refresh_token。过期后需要重新微软登录。',
    cookie_optional_ok:'可选增强',cookie_optional_na:'可选·未注入',
    mode_profile_title:'默认配置',user_tone_hint:'保存后仅影响当前用户，不再跟随全局模板变化。',call_params_title:'调用参数',manual_update_title:'手动更新',status_panel_title:'账户状态',status_account:'账户',status_login:'登录',status_refresh:'续期',status_valid:'Token',status_expire:'过期',status_remaining:'剩余',status_yes:'是',status_no:'否',status_unknown:'未知',account_none:'未绑定',account_none_token:'仅 Token',
    tone_title:'对话模式',model_alias_label:'模型别名',run_permission_label:'运行权限',run_permission_inherit:'继承全局',run_permission_read_only:'只读',run_permission_full:'完全',user_time_zone_label:'更改时区',user_media_suffix_label:'媒体后缀名',user_media_suffix_hint:'填写后将强制覆盖全局媒体后缀，仅作用于你自己的 Key。用逗号、空格或换行分隔。留空则跟随全局。',user_media_suffix_inherit:'全局默认：',ws_idle_timeout_label:'对话响应超时分钟',user_ws_idle_timeout_inherit:'全局默认：',tool_prompt_title:'提示词增强',system_prompt_title:'系统提示词',prompt_card_title:'提示词',click_expand:'点击展开',
  ...
  en:{
    ...
    qs_title:'Quick Start',qs_body:'1. Recommended: click "Microsoft Login" on the account card, sign in, then paste the callback URL that contains code= (no Tampermonkey needed).<br>2. Optional: install the <a href="https://gh-proxy.com/https://raw.githubusercontent.com/MurasameCyan/Ciallo-Ms-365-OpenAI-Proxy-Docker/multi/get_token.user.js" target="_blank" rel="noopener" class="qs-link">Tampermonkey script</a> to push cookies for media/image extras.<br>3. Copy the Base URL and API Key from the account card into your OpenAI-compatible client.',
    ...
    account_title:'Account',push_token_label:'Push / update account token',
    ...
    oauth_title:'Microsoft login (recommended)',oauth_hint:'Click to open Microsoft sign-in. If the page jumps to wrongplace, paste the earlier full URL that still has code=. Status refreshes automatically after success.',oauth_start_btn:'Microsoft Login',oauth_submit_btn:'Submit callback URL',oauth_starting:'Opening...',oauth_submitting:'Submitting...',oauth_ok:'Logged in · auto-refresh ready',oauth_failed:'Login failed',oauth_ph:'Paste https://login.microsoftonline.com/common/oauth2/nativeclient?code=...&state=...',
    login_mode_none:'Not signed in',login_mode_oauth:'Microsoft OAuth',login_mode_rt:'RT auto-refresh',login_mode_cookie:'Cookie session',login_mode_token:'Token only',
    refresh_auto_rt:'Auto (RT)',refresh_auto:'Auto (CDP)',refresh_manual:'Manual',refresh_unavailable:'Unavailable',
    rt_ready:'RT ready · auto-refresh',rt_missing:'No RT · re-login when expired',
    login_ready_hint:'Microsoft account bound. Text chat tokens refresh automatically via RT; you usually do not need to click refresh. Cookie green is only for media extras, not a login failure.',
    login_token_only_hint:'Token present but no refresh_token. You will need to sign in again after expiry.',
    cookie_optional_ok:'optional extras',cookie_optional_na:'optional · not injected',
    mode_profile_title:'Defaults',user_tone_hint:'After saving, this only affects the current user and will no longer follow the global template.',call_params_title:'Call params',manual_update_title:'Manual update',status_panel_title:'Status',status_account:'Account',status_login:'Login',status_refresh:'Renewal',status_valid:'Token',status_expire:'Expires',status_remaining:'left',status_yes:'Yes',status_no:'No',status_unknown:'Unknown',account_none:'Unbound',account_none_token:'Token only',
    tone_title:'Mode',model_alias_label:'Model alias',run_permission_label:'Permission',run_permission_inherit:'Inherit global',run_permission_read_only:'Read-only',run_permission_full:'Full',user_time_zone_label:'Time zone',user_media_suffix_label:'Media suffixes',user_media_suffix_hint:'When filled, this forcibly overrides the global media suffixes and applies only to your own key. Separate with commas, spaces or new lines. Leave empty to inherit the global list.',user_media_suffix_inherit:'Global default: ',ws_idle_timeout_label:'Chat timeout (min)',user_ws_idle_timeout_inherit:'Global default: ',tool_prompt_title:'Prompt boost',system_prompt_title:'System Prompt',prompt_card_title:'Prompts',click_expand:'Expand',
