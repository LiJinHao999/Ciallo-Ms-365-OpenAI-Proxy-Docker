...
def _stored_binding(account) -> tuple[str, str, str, str] | None:
    ...
    if not client_id:
        # Fork-era PKCE accounts stored the issuing client here, not on the
        # later refresh_token_client_id binding fields.
        client_id = str(getattr(account, "oauth_client_id", "") or "").lower()
    ...
    if client_id not in M365_REFRESH_CLIENT_IDS:
        return None
    if not tenant_id or not authority or not object_id:
        ...
        if token:
            ...
            if not tenant_id and isinstance(tid, str) and tid:
                tenant_id = normalize_microsoft_id(tid)
            if not object_id and isinstance(oid, str) and oid:
                object_id = normalize_microsoft_id(oid)
    ...
    if not authority or not tenant_id:
        return None
    if object_id and not account_matches_refresh_subject(account, tenant_id, object_id):
        return None
...
async def refresh_via_rt(accounts: AccountStore, account_id: str) -> bool:
    ...
    binding = _stored_binding(account)
    if binding is None:
        oauth_cid = str(getattr(account, "oauth_client_id", "") or "").lower()
        if oauth_cid in M365_REFRESH_CLIENT_IDS:
            elog(
                f"RT refresh skipped for {account_id}: could not derive "
                "authority/subject from the stored PKCE client; RT kept, "
                "falling back to CDP"
            )
            return False
